...
async function cargarReportes() {
    ...
    if (error) {
        lista.innerHTML = "<tr><td colspan='6' style='text-align:center; color:red;'>Error de conexión. Intente refrescar la página.</td></tr>";
        return;
    }
...
async function actualizarEstado(id, nuevo) {
    await _supabase.from('reportes').update({ estado: nuevo }).eq('id', id);
    cargarReportes();
}